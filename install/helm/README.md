...
```bash
helm install rr -n rr ./install/helm -f custom-values.yaml --create-namespace
```
...
```bash
helm upgrade rr -n rr ./install/helm -f custom-values.yaml
```

## Uninstalling

To uninstall/delete the `rr` deployment:

```bash
helm uninstall rr -n rr
```
...
```bash
kubectl delete pvc -n rr -l app.kubernetes.io/instance=rr
```
...
```yaml
global:
  ownCsUrl: "https://rr-primary.your-domain.com"
...
```yaml
global:
  ownCsUrl: "https://rr-child.your-domain.com"
  centralCsUrl: "https://rr-primary.your-domain.com"
  isChildCluster: true
```
...
| `global.postgresql.tls.verify`         | Verify TLS connection to the service (overrides `tls.verify`)                                       | `false`       |
...
| `global.redis.tls.verify`              | Verify TLS connection to the service (overrides `tls.verify`)                                       | `false`       |
...
| `global.clickhouse.tls.verify`         | Verify TLS connection to the service (overrides `tls.verify`)                                       | `false`       |
...
| `global.grafana.tls.verify`            | Verify TLS connection to the service (overrides `tls.verify`)                                       | `false`       |
...
| Name                       | Description                                            | Value     |
| -------------------------- | ------------------------------------------------------ | --------- |
| `fullnameOverride`         | String to fully override common.fullname               | `rr`      |
...
| `serviceAccount.name`      | Service account name                                   | `rr`      |
...
| Name              | Description       | Value  |
| ----------------- | ----------------- | ------ |
| `metrics.enabled` | Enable rr metrics | `true` |
...
| `postgresql.auth.username`                  | Name of custom user to be created                                                                                                                                                                                                                                                                                                                    | `rr`                |
| `postgresql.auth.password`                  | Password of custom user to be created. Ignored if `auth.existingSecret` is set.                                                                                                                                                                                                                                                                      | `""`                |
| `postgresql.auth.database`                  | Name of custom database to be created                                                                                                                                                                                                                                                                                                                | `rr`                |
...
| `redis.auth.username`                  | Redis username                                                                             | `rr`             |
...
| `rabbitmq.auth.username`                  | RabbitMQ application username                                                                        | `rr`              |
...
| `clickhouse.auth.username`                  | ClickHouse administrator name                                                   | `rr`                  |
...
| `clickhouse.auth.database`                  | Name of the ClickHouse database                                                 | `rr`                  |
