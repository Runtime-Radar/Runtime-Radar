...
func newGWMux(ctx context.Context, grpcAddr string, tlsConfig *tls.Config) (*runtime.ServeMux, error) {
	...
	if err := api.RegisterRoleControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}

	if err := api.RegisterUserControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}

	if err := api.RegisterAuthControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}
