...
func newGWMux(ctx context.Context, grpcAddr string, tlsConfig *tls.Config) (*runtime.ServeMux, error) {
	...
	if err := api.RegisterRuleControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}
	if err := api.RegisterEnforcerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}
