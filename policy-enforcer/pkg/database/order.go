...
func init() {
	sortsMapping = make(map[string]struct{}, len(allowedRuleFields)*len(allowedOrderDirections))
	for _, sort := range allowedRuleFields {
...
func sanitizeOrder(order any) (interface{}, error) {
	if order == nil || order == "" {
		return nil, nil
	}
